...
public class ExportProjectPDF_Extension
{
    ...
    public void UnRegister()
    {
        try
        {
            ...
            MessageBox.Show(
                ...
                "The 'Export PDF Rev' button has been removed from Tools > Scripts.\n" +
    ...
    private void SetupRibbonInterface()
    {
        try
        {
            ...
            // Try to find existing Tools tab using GetTabName() method for consistency
            RibbonTab toolsTab = ribbonBar.GetTab(GetTabName(), true);
            
            if (toolsTab == null)
            {
                // Create Tools tab if it doesn't exist
                toolsTab = ribbonBar.AddTab(GetTabName());
            }
            ...
            // Remove existing commands to prevent duplicates
            try
            {
                foreach (var cmd in scriptsGroup.Commands)
                {
                    if (cmd.Value.ActionCommandLine == "ExportPDFRev" || cmd.Value.ActionCommandLine == "ExportPDFByScheme")
                    {
                        cmd.Value.Remove();
                    }
                }
            ...
            // Add Export PDF Rev button
            scriptsGroup.AddCommand("Export PDF Rev", "ExportPDFRev", new RibbonIcon(CommandIcon.Application));
            // Add Exporty By Scheme button
            scriptsGroup.AddCommand("Exporty By Scheme", "ExportPDFByScheme", new RibbonIcon(CommandIcon.Application));
        }
        catch (Exception ex)
        {
    ...
    private void RemoveRibbonInterface()
    {
        try
        {
            ...
            RibbonTab toolsTab = ribbonBar.GetTab(GetTabName(), true);
            if (toolsTab == null) return;

            RibbonCommandGroup scriptsGroup = toolsTab.GetCommandGroup(GROUP_NAME);
            if (scriptsGroup == null) return;

            // Remove only our specific commands, leave other scripts' commands intact
            try
            {
                foreach (var cmd in scriptsGroup.Commands)
                {
        ...
        {
            MessageBox.Show(string.Format("Ribbon removal error: {0}\n\nStack trace: {1}", ex.Message, ex.StackTrace), "Ribbon Removal Error");
        }
    }

    [DeclareAction("ExportPDFByScheme")]
    public void ExportPDFByScheme()
    {
        try
        {
    ...
    /// <summary>
    /// Prompts the user to select one or more schemes to export
    /// </summary>
    /// <param name="schemes">Array of available scheme names</param>
    /// <returns>Array of selected scheme names, or null if cancelled</returns>
    private string[] PromptForSchemes(string[] schemes)
    {
        using (Form form = new Form())
        {
            form.Text = "Select Schemes to Export";
            form.Width = 350;
            form.Height = 220;
            ...
            label.Text = "Select which PDF schemes to export:";
            label.Location = new System.Drawing.Point(20, 15);
            label.Width = 300;
            form.Controls.Add(label);

            CheckedListBox checkedListBox = new CheckedListBox();
            checkedListBox.Location = new System.Drawing.Point(20, 40);
            checkedListBox.Width = 290;
            checkedListBox.Height = 80;
            checkedListBox.CheckOnClick = true;
            foreach (var scheme in schemes)
                checkedListBox.Items.Add(scheme, true); // Default: all checked
            form.Controls.Add(checkedListBox);

            Button okButton = new Button();
            okButton.Text = "OK";
            okButton.Location = new System.Drawing.Point(140, 140);
            okButton.Width = 75;
            ...
            cancelButton.Location = new System.Drawing.Point(235, 140);
    ...
    /// <summary>
    /// Exports the current project to PDF using the specified filename and scheme
    /// </summary>
    ...
    private void ExportProjectToPDFByScheme(string fileName, string schemeName)
    {
        try
        {
            ...
            progress.BeginPart(100, "Exporting PDF by scheme...");
